...
async function generateMindMapFromGPT() {
  ...
  // ✅ ここで finalPrompt を構築
const finalPrompt = `
あなたは行政DXに詳しい地域経営アドバイザーです。
以下は、ある地域の課題とそれに対する住民の考察です。
これをもとに、中心テーマを「${region}：${theme}」とした放射状マインドマップ構造を構築してください。

出力形式は、MindElixir.jsでそのまま描画できる JSON 構造（{ topic, children } 形式）で、
重要な内容を優先的に階層構造で整理し、日本語で3階層以上の深さにしてください。

出力は必ず JSON オブジェクトのみ（コードブロックや説明文なし）でお願いします。
