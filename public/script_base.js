...
console.log("🧾 combinedText:\n", combinedText); // ← 確認ログ（後で削除OK）
  ...
  // ✅ ここで finalPrompt を構築
combinedText += `
以下は、ある地域における課題と住民・関係者による考察です。  
この情報をもとに、中心テーマ「【地域名】：〇〇市、【テーマ】：△△」に対して、MindElixir.js形式（topic, children）でマインドマップ構造を作成してください。

出力条件は以下の通りです：
- JSON形式で出力（コードブロックや説明文は禁止）
- 中心ノードの下に、抽出された課題を基に構成された重要トピックを放射状に展開してください
- 考察内容は各トピックの `children` として適切に接続してください
- 子ノードの階層は最大3階層までとしてください
- 日本語で記述し、文字数は2500文字以内
- 構造不備（末尾の } や ] の不足）は絶対に避けてください

【地域名】：〇〇市  
【テーマ】：△△  

■ 抽出された課題リスト：
${canvasResult.innerText}
■ 考察リスト：
${thinkingContainer にある textarea すべての value を合成した内容}
`;
