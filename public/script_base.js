...
if (generateMindMapGPTBtn) {
  generateMindMapGPTBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const prompt = `
以下は、地域課題と住民の考察です。  
中心テーマを「${region}：${theme}」として、MindElixir.js形式（topic, children）で放射状のマインドマップ構造を作成してください。

---

【出力仕様】

- 出力は JSON オブジェクトのみ（コードブロック \`\`\` 不要）
- 出力形式は {"topic": "...", "children": [...]} に準拠してください
- "children" を活用して、**階層は最大4〜5階層まで**展開してください
- 各階層で2〜4ノードを目安に構成してください
- 文字列はすべてダブルクオート "..." で囲んでください
- 出力は構文エラーのない **正しいJSON形式で、最後まで閉じてください（}]）**
...
【課題リスト】  
${window.latestExtractedTasks.map((task, i) => `・${task}`).join("\n")}
...
- 出力に「課題【1】：」などの形式は不要です
- 表現は柔軟で構いませんが、必ず構文エラーのないJSONで出力してください
- テーマを中心に放射状に展開し、関連する要素を自然に分類してください
`;
