...
async function generateMindMapFromGPT() {
  ...
  // ✅ ここで finalPrompt を構築
const finalPrompt = `
以下は、ある地域における重要な課題リストと、それに対する住民・関係者による考察です。

これらをもとに、中心テーマを「${region}：${theme}」とした放射状マインドマップ構造を構築してください。

- 各【課題】を中心ノードの1階層目（主枝）として扱ってください。
- それぞれの課題に対して、考察から読み取れる要素を深掘りし、2階層目・3階層目に分岐させてください。
- MindElixir.jsで描画可能なJSON構造（topic, children）形式で構成してください。
- 出力は必ずJSONオブジェクト「のみ」で返してください。コードブロックや説明文は不要です。
