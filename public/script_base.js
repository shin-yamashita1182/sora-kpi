...
const prompt = `${region}周辺の高速道路関連施設について、次の条件でリストアップしてください：

1. 主なインターチェンジ（IC）、サービスエリア（SA）、パーキングエリア（PA）を最大5件まで記載してください。
2. 各施設について、位置関係（例：隣接するICや都市名）、特徴（トイレ、飲食、ガソリンスタンド等）を簡潔に示してください。
3. 重要な施設（大規模SAや地域のハブとなるICなど）が漏れないよう、特に代表的な施設を優先してください。
4. 表現は簡潔かつ網羅的に、リスト形式でお願いします。`;
