...
if (generateMindMapGPTBtn) {
  generateMindMapGPTBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const prompt = `
以下は、地域課題と住民の考察です。  
中心テーマを「${region}：${theme}」として、MindElixir.js形式のマインドマップ（topic, children）構造を使って、**最大5階層の放射状構造**を作成してください。

---

▼条件（厳守）：

- 出力は **JSONオブジェクトのみ**（構文エラー厳禁）
- コードブロック（\`\`\`や説明文）は **一切禁止**
- 文字列は **ダブルクオーテーション**で正確に囲むこと
- 階層構造は **最大5階層**まで展開（3〜5階層混在OK）
- **各ノードに子要素が最低1つ**はある構造にすること（空childrenは不可）
- 子ノード数は階層に応じて **最大2〜3つ程度**
- 出力文字数は **2000文字以内**
- 最後の **} や ] は必ず閉じる**
