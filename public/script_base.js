...
async function generateMindMapFromGPT() {
  ...
  // ✅ ここで finalPrompt を構築
  const finalPrompt = `
以下は、地域課題と住民・関係者からの考察です。これをもとに、中心トピックを「${region}：${theme}」とした放射状マインドマップ構造を設計してください。

出力形式は以下に厳密に従ってください：
- トップレベルには "topic" キーがあり、その値を中心トピック名（例：長崎市琴海町：給食課題）とします。
- その下に "children" を持ち、それぞれが重要な対策の柱になります。
- 各柱も "topic" を持ち、さらに "children" を持ち、最大3階層以上にしてください。
- JSON形式で返してください。絶対にコードブロック（```）や説明文を付けないでください。
