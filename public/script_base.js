...
上記の情報をもとに、マインドマップ構造を生成してください。

- 中心ノードには「${region}」を設定してください。
- 【課題リスト】の10件を、必ず第一階層（中心ノード直下）に展開してください。
- 各課題について、原因・背景 → 解決の方向性 → 想定される施策 などを含めて3〜4階層で構成してください。
- 【住民の考察】は補足情報として、各課題の下層に必要に応じて反映してください。
- 出力は MindElixir.js で描画可能な JSON 形式（topic, children）のみとしてください。
- **コードブロック（\`\`\`）や説明文、余計なテキストは含めないでください。JSONオブジェクトのみを返してください。**
`;
