...
async function generateMindMapFromGPT() {
  ...
  // ✅ ここで finalPrompt を構築
const finalPrompt = `
以下は、ある地域に関する【課題リスト】と【住民の考察】です。

あなたは、地域課題の構造化専門家として、【課題リスト】を最も重視し、階層的なマインドマップ構造を構築してください。

- 中心ノードには「${region}：${theme}」を設定してください。
- 【課題リスト】の10件を、必ず第一階層（中心ノード直下）として配置してください。
- 各課題に対して、原因・背景 → 解決の方向性 → 想定施策例の順に深掘りし、3〜4階層のツリー構造にしてください。
- 【住民の考察】（ThinkingZoneに入力されたもの）は、**各課題を深掘りする際に補足的に使用してください。**
- **ファイルアップロードされた自由記述や擬似録（本文）は中心構造に含めないでください。**
