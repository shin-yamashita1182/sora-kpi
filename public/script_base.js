...
console.log("🧾 combinedText:\n", combinedText); // ← 確認ログ（後で削除OK）
  ...
  // ✅ ここで finalPrompt を構築
combinedText += `
以下は、ある地域における重要なテーマに関するマインドマップを構造的に整理する指示です。

地域とテーマだけを手がかりに、MindElixir.js形式で階層的なマインドマップ（topic, children構造）を作成してください。

- 出力はJSONオブジェクトのみ（コードブロックや注釈は一切不要）
- 最上位ノードは「${region}：${theme}」とし、直接の子ノード（10個程度）にテーマに関する主要課題や切り口を配置してください
- 各課題の下には、具体的な対策や視点（住民・行政・事業者など）を含めて2～3階層構造で展開してください
- 日本語で記述し、記号やナンバリングは不要
- トピック名は20文字以内を推奨し、簡潔にしてください
- 文字数全体は必ず2,500文字以内に収めてください
- 最後の } や ] を欠けなく正確に閉じてください
