...
MindElixir.js で描画可能な JSON 構造（"topic" と "children" を持つ階層構造）として出力してください。
すべて日本語で記述してください。重要な項目は深掘りし、**できるだけ3階層以上の構造にしてください**。

🔒【制約】以下を厳守してください：
- 出力は **JSONオブジェクトの中身のみ**（コードブロックや説明文は一切含めない）
- **必ず有効なJSON構造で、すべての { } と [ ] を閉じて終了すること**
- 途中で省略せず、**最後の閉じ括弧までを返すこと**
- {"topic": "...", "children": [...] } の形式を再帰的に用いること
- **"node" など不要なキーは含めない**
